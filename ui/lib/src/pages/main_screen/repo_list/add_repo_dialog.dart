...
class _AddRepoDialogState extends State<AddRepoDialog> {
  ...
  Future<bool> _initFromRemote() async {
    ...
    try {
      final repo = await initFromRemote(remote: remote, targetDir: target);
  ...
  Future<void> _pickTargetDir() async {
    try {
      final selected = await FilePicker.getDirectoryPath(dialogTitle: "Select destination directory");
      if (selected != null) {
